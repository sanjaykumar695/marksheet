...
def extract_marksheet_details(image_path: str) -> Dict[str, object]:
    ocr = PaddleOCR(use_angle_cls=True, lang='en', show_log=False)
    results = ocr.ocr(image_path, cls=True)

    if not results or not results[0]:
        return {"Medium": "Unknown", "Class": "Unknown", "Student Name": "Not Found", "Marks": {}}
    ...
    # ---------------- Marks Extraction ----------------
    marks: Dict[str, str] = {}
    subject_line_re = re.compile(r"(?i)(ENGLISH|TAMIL|MATHEMATICS|MATHS|SCIENCE|SOCIAL\s*SCIENCE|HINDI|PHYSICS|CHEMISTRY|BIOLOGY)")

    for ln in lines_raw:
        if subject_line_re.search(ln):
            parts = ln.split()
            # try last part as marks (numeric)
            total_marks = None
            for token in reversed(parts):
                if token.isdigit():
                    ...
                    break
            if total_marks:
                subject = " ".join([p for p in parts if not p.isdigit() and len(p) > 1])
                subject = subject.strip()
                marks[subject] = total_marks

    return {"Medium": medium, "Class": marksheet_class, "Student Name": name, "Marks": marks}
